...
export interface UpdateDeluxeRoomTemplateParams {
  name?: string;
  description1?: string;
  description2?: string;
  capacity?: string;
...
export const updateDeluxeRoomTemplate = async ({
  name,
  description1,
  description2,
  capacity,
  ...
  offerPrice,
  policies,
}: UpdateDeluxeRoomTemplateParams) => {
  const template = await prisma.whatsapp_templates.findUnique({
    where: { template_name: "deluxe_room" },
  });

  if (!template || !template.template_json) {
    throw new Error("Template 'deluxe_room' not found");
  }
