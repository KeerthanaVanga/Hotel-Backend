...
export interface UpdateExecutiveRoomTemplateParams {
  name?: string;
  description1?: string;
  description2?: string;
  capacity?: string;
...
export const updateExecutiveRoomTemplate = async ({
  name,
  description1,
  description2,
  capacity,
  ...
  offerPrice,
  policies,
}: UpdateExecutiveRoomTemplateParams) => {
  const template = await prisma.whatsapp_templates.findUnique({
    where: { template_name: "executive_room" },
  });

  if (!template || !template.template_json) {
    throw new Error("Template 'executive_room' not found");
  }
